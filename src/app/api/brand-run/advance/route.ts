...
export async function POST(req: Request){
  ...
  try{
    // fetch current
    const cur = await fetch(`${process.env.APP_URL ?? ''}/api/brand-run/current`, { cache:'no-store' }).then(r=>r.json()).catch(()=>null)
    ...
    const up = await fetch(`${process.env.APP_URL ?? ''}/api/brand-run/upsert`, {
