...
async function apolloDiscovery(params: any) {
  console.log('Calling Apollo with domain:', params.domain);
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Apollo: ${response.status} - ${errorData}`);
    }
...
async function exaDiscovery(params: any) {
  console.log('Calling Exa with domain:', params.domain);
  try {
    const searchTerms = [
      params.brandName,
      params.domain,
    ...
    if (!response.ok) {
      const errorData = await response.text();
      throw new Error(`Exa: ${response.status} - ${errorData}`);
    }
