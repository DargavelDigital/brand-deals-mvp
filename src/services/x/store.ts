...
export async function saveXConnection(conn: XConn) {
  ...
  cookieStore.set('x_conn', JSON.stringify(conn), { httpOnly: true, secure: true, sameSite: 'lax', path: '/', maxAge: 60*60*24*30 })
}
...
export async function loadXConnection(workspaceId: string): Promise<XConn | null> {
  ...
  const cookieStore = await cookies()
  const c = cookieStore.get('x_conn')?.value
  if (!c) return null
  try {
