...
export async function saveLinkedInConnection(conn: LinkedInConn){
  ...
  cookieStore.set('li_conn', JSON.stringify(conn), { httpOnly:true, secure:true, sameSite:'lax', path:'/', maxAge:60*60*24*30 })
}
...
export async function loadLinkedInConnection(workspaceId: string): Promise<LinkedInConn|null>{
  ...
  const cookieStore = await cookies()
  const c = cookieStore.get('li_conn')?.value
  if (!c) return null
